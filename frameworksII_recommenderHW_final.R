...
dim(product_ratings) # 362105 rows 
head(product_ratings)
...
product_ratings %>%
  filter(UserID == "u10023", Product == "prod_14") # Rating is 4  
...
mean(getRatings(train[,'prod_100'])) # average rating is 2.824095

norm_train <- normalize(train)

mean(getRatings(norm_train[,'prod_100'])) # normalized avg rating is 0.08174056
